...
### Enable Grafana Cloud Observability (OTLP)

You can automatically push industry-standard metrics (Time-To-First-Token, Time-Per-Output-Token, CPU/VRAM utilization, and Prefix Cache hits) and all vLLM logs securely to Grafana Cloud using OpenTelemetry (OTLP).

Recommended environment variables:

```bash
OTEL_SERVICE_NAME="sql-egine"
OTEL_EXPORTER_OTLP_ENDPOINT="https://otlp-gateway-prod-ap-south-1.grafana.net/otlp"
OTEL_EXPORTER_OTLP_PROTOCOL="http/protobuf"
OTEL_EXPORTER_OTLP_HEADERS="Authorization=Basic <YOUR_BASE64_AUTH>"
```

What these do:
- `OTEL_SERVICE_NAME`: The identifier for this worker in Grafana (e.g. `sql-egine`).
- `OTEL_EXPORTER_OTLP_ENDPOINT`: The unified OTLP endpoint from your Grafana Cloud connections.
- `OTEL_EXPORTER_OTLP_HEADERS`: The authentication token generated by Grafana Cloud.

When configured, the worker runs a background thread that relays GPU usage and pushes all request metrics (TTFT, internal queues) automatically every 15 seconds.
